...
with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options), graph=loaded_graph) as sess:
    ...
    for i in tqdm.tqdm(range(len(source_text_to_int)//batch_size)):
        ...
        for j in range(batch_size):
            fr_preds.append(" ".join([target_int_to_vocab[word_idx] for word_idx in translate_logits[j]]))
    ...
    for k in tqdm.tqdm(range(len(fr_preds))):
        # 去掉特殊字符
        pred = fr_preds[k].replace("<EOS>", "").replace("<PAD>", "").rstrip()
        reference = references[k].lower()
        if k%10000 == 0:
            print("pred:", pred)
            print("reference:", reference)
        # 计算BLEU分数
        score = sentence_bleu([reference.split()], pred.split(), weights=[0,0,0,1])
